...
def update_state(state: State):
    ...
    state.df_selected_company = state.data[state.data["mnc"] == state.selected_company]
    ...
    state.selector_year = state.df_selected_company["year"].unique().astype(str).tolist()
...
def update_viz_year(state: State):
    ...
    state.viz[id] = Viz(id=id, 
                        state=state, 
                        data=algo.display_transparency_score(state.data, state.selected_company, int(state.selected_year)),
                        title="Transparency Score",
    ...
    state.viz[id] = Viz(id=id,
                        state=state,    
                        data=algo.display_transparency_score_over_time_details(state.data, state.selected_company),
    ...
    state.viz[id] = Viz(id=id,
                        state=state,
                        data=algo.display_company_key_financials_kpis(state.data, state.selected_company, int(state.selected_year)),
                        title="Key metrics",
                        sub_title=f"selected fiscal year : {state.selected_year}"
                    ).to_state()
    # print(f'update viz id:{id} title:{state.viz[id].title}')
    ...
    state.viz[id] = Viz(id=id,
                        state=state,
                        fig=algo.display_jurisdictions_top_revenue(state.data, state.selected_company, int(state.selected_year)),
    ...
    state.viz[id] = Viz(id=id,
                        state=state,
                        fig=algo.display_pretax_profit_and_employees_rank(state.data, state.selected_company, int(state.selected_year)),
    ...
    state.viz[id] = Viz(id=id,
                        state=state,
                        fig=algo.display_pretax_profit_and_profit_per_employee(state.data, state.selected_company, int(state.selected_year)),
